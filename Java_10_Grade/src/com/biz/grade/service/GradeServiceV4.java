...
public class GradeServiceV4 {
	private int[] intKor, intEng, intMath, sum, avg;
	private Scanner scan;

	// 생성자 만들기. public 클래스와 같은 메소드 이름. return 타입 없음

	public GradeServiceV4(int length) {
		...
		for(int i = 0; i < sum.length; i++) {
			for(int j = i + 1; j < sum.length; j++) {
				if(sum[i] < sum[j]) {
					...
					// if가 맞다면 Kor, Eng, Math의 값들도 i와 j를 temp를 사용하여 교체시킨다
					// 배열은 이것을 하나의 덩어리로 묶을 수가 없다
					temp = intKor[i];
					intKor[i] = intKor[j];
					intKor[j] = temp;
					temp = intEng[i];
