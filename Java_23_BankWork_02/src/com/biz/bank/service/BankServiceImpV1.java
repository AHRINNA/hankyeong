...
public class BankServiceImpV1 implements BankService {
	...
	public void readBook(String bookFile) throws Exception {
		...
		while (true) {
			...
			/*
			 * books[0] : 계좌정보 books[1] : 최종잔액 books[2] : 최종거래일자 books[3] : 구분
			 */
	...
	/*
	 * 키보드로 계좌번호를 입력받아서 계좌번호가 list에 있으면 계좌정보(VO)를 추출하여 임시 보관하기
	 */
	@Override
	public void viewAcc() {
		// TODO 계좌정보조회
	...
	public void input() throws Exception {
		...
		// 구분문자열(remark)과 입금된 금액을 파라미터로 전달
		this.accListWriter(bookVO.getRemark(), intInput);

		this.bookWrite(bookFile);
		// 입금처리가 완료된 후 원장에 내용을 저장하는 bookWrite 메소드 호출
		// 여기서 문제가 발생하는데 저장할 파일이름을 넘겨줄 방법을 모색해야한다
		// input()을 변경하여 input(String bookFile)로 바꿔도 되지만
		// interface, interface를 implement한 클래스 모두를 변경해야하기 때문에
		// 최소한의 코드를 변경하여 원하는 결과를 얻기 위해 Service에 bookFile field를 선언하고
		// 절차상 먼저 실행되어야 할 메소드에서 필드에 값을 세팅하여 이를 활용하도록 설정한다
	...
	/*
	 * 1. 앞에서 조회된 계좌번호가 유효하지 않으면 method 종료 bookVO가 null 이면 계좌번호가 유효하지 않다. 2. 키보드로
	 * 출금액을 입력받고 3. 음수이면 출금 거부!! 입력받은 값 < 0 이면 거부 4. 잔액 >= 출금 이면 출금 처리 5. 아니면 출금 거부
	 */
	@Override
		...
		if (bookVO != null) {
			...
			if (intOutput > 0) {
				...
				if (intBalance >= intOutput) {
					...
					this.accListWriter(bookVO.getRemark(), intOutput);
					this.bookWrite(bookFile);
				} else {
	...
	/*
	 * 입,출금이 완료되는 순간 bookFile에 변동 내역 저장
	 * 
	 * bookFile 변수를 매개변수로 받아 변수의 내용을 파일명으로 하는 처리를 수행해야 한다
	 * 
	 */
	@Override
	public void bookWrite(String bookFile) throws Exception {
		...
		for (String key : keys) {
			pw.printf("%s:%d:%s:%s\n", bankBookList.get(key).getNum(), bankBookList.get(key).getBalance(),
					bankBookList.get(key).getLastDate(), bankBookList.get(key).getRemark());
			pw.flush();
		}
		pw.close();
	}
	...
	/*
	 * 어떤 계좌에서 입금이나 출금이 발생하면 내역을 계좌번호파일에 저장
	 */
	@Override
	public void accListWriter(String remark, int money) throws Exception {
		// TODO 거래내역을 파일로 저장
		String strAccPath = "src/com/biz/bank/books/";
		FileWriter fileWriter;
		PrintWriter accBook;
		// 계좌번호로 거래내역을 저장할 파일이름을 생성. BOOK-0001
		String accBookFile = String.format("BOOK-%s.txt", bookVO.getNum());

		fileWriter = new FileWriter(strAccPath + accBookFile, true);
		// src/com/biz/bank/acc/BOOK-0001.txt 형식의 파일이름으로 파일 생성
		// filewriter(,true). append mode
